...
You may assume that `nums[-1] = nums[n] = -∞`, meaning an element is always considered **greater** than an out-of-bounds neighbor.  
...
**Hints:**  
1. If `nums[mid] > nums[mid + 1]`, then the **peak lies on the left half** (including `mid`).  
2. Otherwise, the **peak lies on the right half**.  
3. Use **binary search** to achieve `O(log n)` complexity.  

[📂 Solution](Solutions/0162.java)  
...
2. For each spell, find the **least strength potion** that will form a successful pair.  
3. **Sort** the potions based on strength and use **binary search** to efficiently count the valid pairs.  

[📂 Solution](Solutions/2300.java)  
